...
import { Flame, Copy, Check, Filter, Zap } from 'lucide-react';
import { cn } from '@/lib/utils';

export default function TruthOrDareHubClient({ locale }: { locale: string }) {
  ...
  return (
    <>
      ...
      />
          ...
          <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border-2 border-purple-200 dark:border-purple-900/50 shadow-xl space-y-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-purple-100 dark:border-slate-800 pb-4">
              <div>
                ...
                {/* Visual Category Landing Link for SEO */}
                <div className="mt-2">
                  <Link href={`/${locale}/truth-or-dare-questions/${selectedCategory.id}/`} className="text-xs font-bold text-purple-600 hover:underline">
                    Go to dedicated {selectedCategory.name} Page &rarr;
                  </Link>
                </div>
              </div>
        ...
        {/* Explore Related Guides & Articles Section */}
